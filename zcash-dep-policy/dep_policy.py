#!/usr/bin/env python3
"""Zcash dependency-version policy checker (shared toolbox script).

Keeps a consumer repo's governed Zcash crate pins aligned with their upstream
libraries, and reports drift. Driven entirely by a per-repo
``governed-libs.toml`` config, so the same script serves every consumer repo
(see this directory's README and examples/).

The source of truth for a governed library's version is the library's own
upstream repository at its default-branch HEAD. A consumer can depend on a
governed library in one of two ways, and each is checked accordingly:
